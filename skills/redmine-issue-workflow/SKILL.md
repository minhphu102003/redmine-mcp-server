...
1. **Live data first** — never trust memory, tool-description defaults, or this skill's examples. Exception: a fresh `.redmine` cache (≤ 14 days) is a trusted fast path for its static lists — the exact decision tree is in Step 1.0. Anything missing from the cache, or dynamic (allowed status transitions, parent-issue validity), must be fetched live. Stale cache → warn and suggest `redmine init`; no cache → live lookups (the `redmine-init` skill creates it).
...
**Install-time config — the only value to fill in this skill:** `{{COMMIT_WORKFLOW_PATH}}` (path relative to the **root of the repository where this skill is installed** — the same repo the issues belong to):
...
0. **Ask the user for the data source first**: base the issue on (a) a **PR/commit on the remote** (PR number or commit SHA), or (b) the **current uncommitted changes** in the local working tree?
   ...
   **(b) Local uncommitted changes** — `git status` + `git diff --stat` / `git diff` for the files changed and their content. There is **no commit subject** → draft one from the change or ask the user. Author comes from `git config` (mapped in 3a).

- Don't parse `gh api --paginate` output — clone and use `git log` instead.

---

## Step 3 — Map commit data to the issue

### 3a. Commit author → Redmine member
...
## Step 5 — Update an existing issue from a PR/commit

Use when the user provides an existing issue ID, or the mode rules in Rule 7 match an existing issue by subject prefix (e.g. `[FE] Add PDF parsing` matches a PR whose latest commit subject is `Add PDF parsing`). Read the **latest commit/PR only** — never full git history.

### 5a. Find the existing issue
...
3. **Already updated?** If the issue's description already has a `## Changelog` entry for this PR number or commit SHA → tell the user it was already updated; proceed only with the parts the user explicitly asks for (e.g. log time).

### 5b. Read the latest PR/commit
...
### 5c. Proposed update actions (confirm with the user in 5e)
...
| **Description** | **Append** a changelog entry (5d) — never replace the existing description |
| **Time log** | merged/final commit → propose **actual hours** (ask the user; fallback: the issue's `estimated_hours`); `spent_on` = latest commit date; confirm with the user |
| **Other changes** (assignee, priority, version, subject) | Only when the user asks or the PR/commit clearly implies them — propose and confirm, never silently change |

### 5d. Changelog entry format
...
### 5e. Confirm with the user

Present: issue ID + current subject, proposed status (live options), proposed done_ratio, proposed time log (hours, date), changelog entry preview — per Rule 4 (apply adjustments and proceed).

### 5f. Execute the update

1. Update the issue with the confirmed fields (`status_id`, `done_ratio`, `description` with appended changelog).
2. If time logging is confirmed → get valid `activity_id` values from the time-activity list tool first, then create the time entry (`issue_id`, `hours`, `activity_id`, `spent_on`, `comments`).
3. **Read-only server** (`REDMINE_MCP_READ_ONLY`): updates are blocked → present the full change list for manual application in the UI.
4. Invalid status transition → fetch allowed statuses and suggest the nearest valid one.
5. Report: issue ID, subject, updated status, done_ratio, time logged (if any), changelog entry appended.

---

## Step 6 — Description template (English, fixed format)

You (the agent) draft the entire description from the commit/PR/repo context (actual code, files, changes); the user only reviews/edits it at confirmation (Step 4, step 1). Fill all 8 sections fully in English:
...
- Append the PR link on the last line; no PR yet (uncommitted working-tree changes) → end with `Commit: <sha>` (or `Commit: working-tree changes`) instead — never invent a PR URL.
