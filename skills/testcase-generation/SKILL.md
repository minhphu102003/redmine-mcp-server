...
11. **Output limit (max ~10 detailed TCs per turn)**: avoid generating too much output in one turn. If the US requires more than 10 test cases, generate detailed versions for the first 10 (prioritize happy path + critical edge cases), and write outlines for the rest. Ask the user to request more detail in subsequent turns.
...
### Presentation message (after writing draft)

```
Draft saved to: .tmp/testcases-draft.md

Generated detailed: 10 test cases (from AC1-3, Business Rule 1)
Outline (awaiting detail): 5 test cases

Outline:
- TC-11: <title> (from AC4)
- TC-12: <title> (from Business Rule 1)
- TC-13: <title> (from AC5)
- TC-14: <title> (from Business Rule 5)
- TC-15: <title> (edge case)

To continue, you can say:
- "detail TC-11, TC-12" → I will write the full version of these 2 cases
- "detail all outlines" → I will do them in turns (max 10 per turn)

Once complete, say "approve" → I will push to Google Sheet.
```
...
1. Re-read `.tmp/testcases-draft.md`.
2. Apply changes as requested.
3. Re-write the draft file.
4. Re-present the summary.

**b) User wants to detail from outline** ("detail TC-11", "detail all outlines"):
1. Re-read draft file, find the outlines to detail.
...
6. Update the "Detailed: X, Outline: Y" summary.
...
1. Re-read `.tmp/testcases-draft.md` one final time.
...
6. **Report**:
   ...
   - Draft file: .tmp/testcases-draft.md (can be deleted)
   ```
...
- [ ] Draft file goes to `.tmp/testcases-draft.md` — not the sheet directly.
- [ ] IDs are generated at push time, not in the draft.
- [ ] Always re-read the draft file before pushing (user may have edited it externally).
