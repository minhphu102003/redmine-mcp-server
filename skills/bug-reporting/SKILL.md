...
1. **Memory check first**: call `get_user_memory(key=".redmine")` and `get_user_memory(key=".google-sheets")` before asking anything.
   - **Source**: server-side memory via MCP tools
...
**Memory check first** (both must exist after init):

1. Call `get_user_memory(key=".redmine")` → `user_mappings[0].redmine_name` → auto-fill `reporter`. If missing → init was not run, tell user to run `redmine init` first.
2. Call `get_user_memory(key=".google-sheets")` → find mapping for current project (match `redmine_project_id` against `.redmine` `project.id`).
...
- [ ] **Read `.redmine` first** — reporter is always auto-filled, never ask. Use `get_user_memory(key=".redmine")`.
- [ ] **Read `.google-sheets` first** — auto-fill spreadsheet_id and sheet name, never ask if mapping exists. Use `get_user_memory(key=".google-sheets")`.
