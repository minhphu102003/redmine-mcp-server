...
10. **Read-only guard**: if the server is in read-only mode (`REDMINE_MCP_READ_ONLY`), creation tools return an error — detect it early and tell the user Checkpoint 2 cannot write; offer to finish Checkpoint 1 (proposal + state) only.
...
1. Present the full draft tree (subjects + descriptions + estimates + assignee/priority proposals, totals per epic).
2. Apply every adjustment the user makes and re-present — **repeat until the user explicitly confirms the proposal**. Never skip to Checkpoint 2 on your own judgment.
