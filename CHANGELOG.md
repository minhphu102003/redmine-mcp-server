...
- **`redmine-planning` agent skill**: a user-facing skill at `skills/redmine-planning/` that turns a natural-language goal or meeting notes into a structured Redmine plan (Epic → Story → Task) in two checkpoints: (1) build and confirm a user-story proposal (state persisted as JSON in the `.redmine` plan section, nothing written to Redmine yet); (2) after the user's go-ahead, ground the plan in repository architecture read ONLY from `AGENTS.md`/`CLAUDE.md`/`ARCHITECTURE.md` (never scanning the repo), break each story into tasks with estimates from complexity and assignees from module ownership, collect dependencies as `precedes`/`follows` issue relations, then bulk-create epics, stories-with-subtasks and relations. Includes an update mode (add tasks to a story, change estimates/version/priority, add/remove dependencies) and a README with installation steps
