...
- **`redmine-issue-workflow` uses the agent's structured ask UI**: param confirmations now go through the agent's ask/choice tool when available (opencode `question`, Claude Code `AskUserQuestion`, Codex `request_user_input`) with live option lists, batching 1–4 questions per call; full option lists are embedded as numbered text in the question and answered by typing the number/name (custom/free-text input), with up to 4 clickable shortcuts of the most likely picks, plus a plain-text fallback for agents without an ask tool and for non-interactive sessions (e.g. Codex `exec`)
