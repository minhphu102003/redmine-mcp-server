...
## [Unreleased]
### Added
- **Personnel & performance tools (boss oversight, read-only)**: `list_personnel` (unique project members across projects with projects+roles, group memberships skipped, per-project errors kept — boss step 1: pick a person) and `get_person_work_summary` (one person's day or Mon–Sun week performance grouped by project: hours logged, issues touched/closed in the window, current backlog with fixed overdue rule `open status + due_date < today`, plus an `evidence` block with filters/query time/totals for Redmine-UI cross-checks — boss step 3; the agent renders the UI). Person resolution uses the admin `/users.json` API (ambiguous names return candidates, never guessed). Tool count 37 → 39
- **`boss-project-oversight` agent skill** at `skills/boss-project-oversight/`: 3-step flow (personnel list → ask day-or-week verbatim → grouped summary with evidence footer), one person per run, read-only, no memory tools. Wired into `install-skills.ps1`, `install-skills-user.ps1`, and the Claude Desktop ZIP installer
- **Oversight widget data + skill spec**: `get_person_work_summary` gains `compact` mode (person + window + `widget_data` + totals + evidence only), per-issue `estimated_hours`/`actual_hours` (actual joins time entries on that issue inside the window), a `completed` rule (`done_ratio == 100` updated in the window, even with an open status), and `widget_data` keyed `Thứ 2`…`Chủ nhật` with `{id, name, project, est, actual, url}` items (2-decimal rounding) ready to embed verbatim; the skill's Step 3 is template-first (reads sibling `widget-template.html` before rendering, slots-only edits, version pin, pre-emit checklist) with a compact embedded fallback when the file is absent
- **Installers ship the boss widget template**: shared `Install-Skill` helpers accept generic `-ExtraExtensions` (default empty — other skills unchanged); the per-repo, user-level, Claude Code, and Claude Desktop installers pass `@('.html')` for `boss-project-oversight` only, so `widget-template.html` lands next to `SKILL.md` (warn-and-continue when the file does not exist yet)
...
- **`scripts/install-skills-codex.ps1`** — new local installer that copies **all** skills under `skills/` (auto-discovered via `SKILL.md`, so future skills need no script change) to `$env:USERPROFILE\.agents\skills` for Codex auto-loading. Ships `.html` payloads when a skill has them (boss templates), skips `README.md`, idempotent.

### Changed
- **`install-skills.ps1` / `install-skills-user.ps1` / `install-skills-claude-desktop.ps1` now include `boss-project-oversight`** alongside the existing skills
- **All six `install-skills*.ps1` scripts now use the shared `Install-Skill` helper** — the hardcoded per-skill extra-file lists (`member-rules-catalog.md`, `google-sheets-schema.md`, `USER_STORY_TEMPLATE.md`) are gone. New `*.md` files added under `skills/<skill>/` are picked up automatically on the next install. `README.md` is filtered out everywhere (previously Claude Code and Claude Desktop ZIP installers were including it).
- **`install-skills-claude-code.ps1` no longer copies `README.md`** — only `SKILL.md` and the other `*.md` template files land in `~/.claude/skills/<skill>/`. Same/timestamped-update logic is preserved.
- **`install-skills-claude-desktop.ps1` ZIP packages no longer include `README.md`** — the Contents API filter now skips `README.md` so the uploaded ZIP contains only the skill payload.

### Added
- **Issue relation tools**: `create_redmine_issue_relation` (create a dependency between two issues — `precedes`/`follows` with optional `delay`, `blocks`/`blocked`, `relates`, `duplicates`/`duplicated`, `copied_to`/`copied_from`; validates the relation type, rejects self-relations, verifies both issues exist before creating, wraps subject text as insecure content) and `delete_redmine_issue_relation` (delete an existing relation by ID). Both respect read-only mode
- **`redmine-planning` agent skill**: a user-facing skill at `skills/redmine-planning/` that breaks **one user story** down into tasks with the right assignees (optionally via lower-level sub-stories) in two checkpoints: (1) build and confirm the story's business proposal — every ambiguous point (context, role, capability, value, acceptance criteria) is resolved by the user, never self-interpreted, and technically infeasible requirements are asked back (state persisted as JSON in the `.redmine` plan section, nothing written to Redmine yet); (2) after the user's go-ahead, ground the breakdown in repository architecture read ONLY from `AGENTS.md`/`CLAUDE.md`/`ARCHITECTURE.md` (never scanning the repo), break the story into tasks with estimates = implementation time + 20 % buffer (leaf tasks 4–8 h, each justified with the risks it covers) and assignees from module ownership, collect dependencies as `precedes`/`follows` issue relations, then create the story, sub-stories and tasks. Scope is ONE story per run — whole-sprint planning is declined (context overflow) and the plan always belongs to the repo's project (planning another repo is refused). Includes an update mode (add tasks to a story, change estimates/version/priority, add/remove dependencies) and a README with installation steps
- **Skill installer script**: `scripts/install-skills.ps1` installs the agent skills into a target repository's `.agents/skills/` (only the `SKILL.md` files, no READMEs) so every agent (opencode, Agent SDK) can use them. One-liner from any repo: `irm https://raw.githubusercontent.com/minhphu102003/redmine-mcp-server/develop/scripts/install-skills.ps1 | iex`; running the script from a local clone copies from the local `skills/` folder instead of downloading
- **`redmine-issue-workflow` agent skill**: a user-facing skill at `skills/redmine-issue-workflow/` that teaches AI agents the exact workflow for creating a Redmine issue from a GitHub commit (live-data verification, author→member mapping, `[FE/BE/Devops]` naming, English description template). The folder includes a README with installation steps for opencode plus GitHub CLI (`gh`) install and OAuth login instructions
...
### Changed
- **`redmine-planning` task description template upgraded**: task descriptions are no longer a single paragraph — every task follows a 6-section template: `## Deliverable` (one concrete outcome, "I wrote code" is not done), `## Scope of work` (2–5 implementation steps grounded in the architecture context), `## Acceptance` (1–3 pass/fail checks that trace to the story's acceptance criteria — each AC scenario maps to a task), `## Out of scope` (explicitly names the other task(s) owning the leftover work so two tasks never silently overlap), `## Dependencies` (mirrors exactly the confirmed `precedes` pairs — same list stored as `depends_on` and created as issue relations), and `## Risks` (the problems justifying the estimate). The `Part of story #<ref>` line stays at the top; the schema example and README structure notes were updated to match
- **`redmine-init` asks for member working rules**: during init, for each GitHub↔Redmine-mapped member the skill asks the user for that person's working rules — role/stack (backend, frontend/web, mobile, devops, AI/LLM, data analyst, QA, full-stack, **lead**, data engineer, security, SRE, PM, ...) and personal conventions (required tests, code-review expectations, AI-assistant usage policy, reporting cadence, definition of done). The user can answer or skip per member ("bỏ qua"); nothing is ever invented. Answers are stored in `.redmine` as `member_rules` (per-member `roles` tags + verbatim rule bullets) so planning/issue skills can respect each person's constraints. The skill embeds a researched catalog of rule areas per role (incl. **Lead**: architecture decisions, review/approval gates, delegation, risk ownership) to prompt the user; refresh keeps existing entries and only re-asks when the user asks to update
- **`redmine-planning` proposes assignees from `member_rules`**: when breaking a story into tasks, the assignee proposals now respect each member's stored roles/stack and personal rules (a task is only proposed to someone whose roles fit the work and whose rules don't exclude it — e.g. backend work is never proposed to a frontend-only member, and a lead's review/approval rules surface as a separate review task/check, not as the implementation assignee); no fitting candidate → the user is asked, never silently overridden. The `.redmine` schema example includes the `member_rules` key
- **`redmine-planning` story description template upgraded**: the story template now includes `## Out of scope` (what the story deliberately does NOT cover — prevents scope creep and estimate disputes during task breakdown) and `## Notes & open questions` (dependencies, assumptions, unresolved `[?]` points); acceptance criteria are guided to 3–5 pass/fail conditions covering the happy path plus edge cases (invalid input, authorization, retry, partial failure) with Given/When/Then for state-dependent behavior (each scenario maps to a task in Checkpoint 2); `As a` must name a specific role (never "a user") and `So that` must carry a real business value (delete-test); Checkpoint 1 confirmation (2d) adds a quick quality check on role/value/criteria/out-of-scope before presenting the draft
- **`redmine-planning` skill deduplicated and fixed**: the SKILL.md frontmatter description was cut from ~3000 chars (it was being truncated at 2000 and duplicated the whole skill) to a ~1300-char summary of triggers + scope + key constraints; the checkpoint diagram and rule prose repeated in the body (never-scan-repo, no self-interpretation, estimate rule, dates, one-plan-in-memory, plan-scope, cache, read-only) were consolidated into the core rules, and the gotchas checklist was trimmed to the items not already covered by the rules. The schema example now matches its own tracker IDs (Story = 3, Task = 4 — previously the `decisions` referenced IDs absent from the `trackers` list) and shows a `created` plan with real `redmine_id`s (previously `proposed` with tasks present, contradicting the rule that tasks are only broken down in Checkpoint 2); Checkpoint 2 step 6 now bumps `updated_at` instead of recording `created_at`
- **`get_redmine_issue` and `list_redmine_issues` now return planning fields**: every serialized issue includes `start_date` and `due_date` (`YYYY-MM-DD`, `null` when unset) plus `estimated_hours` and `done_ratio`; `_issue_to_dict_selective` accepts them in the `fields` parameter (they were previously silently skipped, so requesting `["start_date", "due_date"]` returned nothing for those keys)
- **`get_project_issue_context` now returns `priorities`**: the consolidated project context includes the complete list of issue priorities defined in Redmine (`id`, `name`), fetched concurrently with the other sections via the enumerations endpoint (`/enumerations/issue_priorities.json`), so agents can pick a valid `priority_id` before creating an issue without sampling existing issues
- **Priority fallback fix**: `_resolve_priority_id_by_name` (used to set the default priority when `priority_id` is omitted at creation) used `client.issue_priority.all`, a resource python-redmine 2.5 does not register, so the fallback silently failed; it now lists priorities via the enumerations endpoint like the new context section
- **`redmine-issue-workflow` consumes the `.redmine` cache**: Step 1 now checks for a `.redmine` file at the git worktree root and uses its static ID lists as the fast path when `fetched_at` is within 14 days, warning the user and suggesting a `redmine init` refresh when stale; any ID missing from the cache is still verified live
- **`redmine-issue-workflow` deduplicates and maps the tracker by change type**: before creating, the skill searches the target project for an existing matching issue (found → asks the user to update it instead of duplicating); the tracker is chosen by change type (fix/bugfix/hotfix → Bug, otherwise Feature) instead of always defaulting to Feature; required custom fields are asked at confirmation; updates no longer re-append changelog entries for a PR already recorded, and read-only servers get a full text draft for manual creation instead of a silent failure
- **`redmine-issue-workflow` uses the agent's structured ask UI**: param confirmations now go through the agent's ask/choice tool when available (opencode `question`, Codex `request_user_input`) with live option lists, batching 1–4 questions per call; full option lists are embedded as numbered text in the question and answered by typing the number/name (custom/free-text input), with up to 4 clickable shortcuts of the most likely picks, plus a plain-text fallback for agents without an ask tool and for non-interactive sessions (e.g. Codex `exec`)
- **`redmine-issue-workflow` auto-drafts the description and dates**: the agent now generates the full English description from the commit/PR/repo context (template in Section 6) instead of the user writing it from scratch — the user only reviews/edits the draft at confirmation; `start_date`/`due_date` are pre-proposed (commit date, +1 day) and adjustable, never invented by the user
...
- **`redmine-init` stores GitHub↔Redmine account mapping**: during init, the agent detects the current GitHub identity via `gh api user` (falls back to `git config`), matches against the project member list, and stores the mapping in `.redmine` under `user_mappings`. The `redmine-issue-workflow` skill reads this mapping first — if a commit author matches a stored entry, the assignee is set automatically without asking. Optional: map additional committers via `git shortlog -sne --since="6 months"` (compact, few lines). Default: map only the current user to save tokens.
- **`redmine-issue-workflow` supports updating existing issues from PRs/commits**: new Step 5 (Section 6) reads the latest PR/commit (not full git history), finds the matching existing issue by ID or subject prefix, proposes status/done_ratio/description changelog/time log updates, confirms with the user, and executes. Changelog entries are appended to the existing description (never replaced). Time logging uses the issue's `estimated_hours` by default.
- **Agent-facing tool descriptions**: docstrings of core workflow tools (`get_redmine_issue`, `list_redmine_projects`, `get_project_issue_context`, `list_redmine_issues`, `search_redmine_issues`, `create_redmine_issue`, `create_redmine_issue_with_subtasks`, `update_redmine_issue`, `manage_time_entries`) enriched with usage guidance; these are exposed as MCP tool `description`s and are visible to agents even when prompts are not loaded
...
- **opencode MCP client configuration** added to README with stdio transport via FastMCP proxy (NOTE: opencode has compatibility issues - not recommended)
...
- opencode CLI setup command documentation (NOTE: opencode has compatibility issues - not recommended)
...
- opencode users can now use: `claude mcp add --transport http redmine http://127.0.0.1:8000/mcp` (NOTE: opencode has compatibility issues - not recommended)
