...
- **Oversight widget data + skill spec**: `get_person_work_summary` gains `compact` mode (person + window + `widget_data` + totals + evidence only), per-issue `estimated_hours`/`actual_hours` (actual joins time entries on that issue inside the window), a `completed` rule (`done_ratio == 100` updated in the window, even with an open status), and `widget_data` keyed `Thứ 2`…`Chủ nhật` with `{id, name, project, est, actual, url}` items (2-decimal rounding) ready to embed verbatim; the skill's Step 3 now specifies the full interactive HTML widget (project dropdown, 2 metric cards, stacked Mon–Sun bar chart, click-a-day detail table with red/green variance, fixed palette, offline single-file artifact)
