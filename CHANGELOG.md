...
### Changed
- **`redmine-init` asks for member working rules**: during init, for each GitHub↔Redmine-mapped member the skill asks the user for that person's working rules — role/stack (backend, frontend/web, mobile, devops, AI/LLM, data analyst, QA, full-stack, data engineer, security, SRE, PM, ...) and personal conventions (required tests, code-review expectations, AI-assistant usage policy, reporting cadence, definition of done). The user can answer or skip per member ("bỏ qua"); nothing is ever invented. Answers are stored in `.redmine` as `member_rules` (per-member `roles` tags + verbatim rule bullets) so planning/issue skills can respect each person's constraints. The skill embeds a researched catalog of rule areas per role to prompt the user; refresh keeps existing entries and only re-asks when the user asks to update
