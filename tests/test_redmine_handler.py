...
class TestRedmineHandler:
    ...
    @patch("redmine_mcp_server.redmine_handler.list_redmine_issue_statuses_impl")
    @patch("redmine_mcp_server.redmine_handler.get_redmine_project_workflow_impl")
    async def test_workflow_tracker_contract_resource_invalid_tracker_id(
        self, mock_workflow_impl, mock_statuses_impl
    ):
        """Tracker workflow contract should validate tracker_id consistently."""
        from redmine_mcp_server.redmine_handler import (
            workflow_tracker_contract_resource,
        )

        result = await workflow_tracker_contract_resource(5, "not-a-number")

        assert (
            result["error"]
            == "Invalid tracker_id 'not-a-number'. Expected an integer tracker ID."
        )
        mock_workflow_impl.assert_not_called()
        mock_statuses_impl.assert_not_called()
